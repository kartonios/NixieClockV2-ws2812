void flipTick() {
  if (FLIP_EFFECT == 0) {
    ...
    if (flipTimer.isReady()) {
      ...
      if (flipCounter == INDI_COUNT) {   // если ни одну из 4 цифр менять не нужно
        // выходим из цикла изменения
        flipInit = false;
        newTimeFlag = false;
      }
...
//========  RainbowPulseFx  ========//
void rainbowPulse(byte maxBrightness) {
  wsFxTimer.setInterval(pulsePeriod);
  if (colorCounter > 7) colorCounter = 0;
  if (wsFxTimer.isReady()) {
    pulseLedBrigthness += pulseStep;
    strip.setPixelColor(0, setColor(colorCounter));
    strip.setPixelColor(1, setColor(colorCounter));
    strip.setPixelColor(2, setColor(colorCounter));
    strip.setPixelColor(3, setColor(colorCounter));
    strip.setPixelColor(4, setColor(colorCounter));
    strip.setPixelColor(5, setColor(colorCounter));
    strip.setBrightness(pulseLedBrigthness);
    strip.show();
    if (pulseLedBrigthness == maxBrightness) pulseStep = -pulseStep;
    if (pulseLedBrigthness == 0 ) {
      colorCounter++;
      pulseStep = -pulseStep;
    }
  }
...
//========  RandomPulseFx  ========//
void randomPulse(byte maxBrightness) {
  byte lastColorCounter = colorCounter;
  wsFxTimer.setInterval(pulsePeriod);
  if (wsFxTimer.isReady()) {
    ...
    if (pulseLedBrigthness == 0 ) {
      ...
      if (colorCounter == lastColorCounter)      {
        while (colorCounter == lastColorCounter) {
          colorCounter = random(0, 7);
        }
      }
    }
...
void rainbowCycle(uint16_t wait) {
  ...
  if (wsFxTimer.isReady()) {
    ...
    strip.setPixelColor(4, Wheel(((4 * 256 / 30) + rainbowWheel) & 255));
    strip.setPixelColor(5, Wheel(((5 * 256 / 30) + rainbowWheel) & 255));
    strip.show();
    rainbowWheel++;
  }
}
...
void rainbowFade(uint16_t wait) {
  ...
  if (wsFxTimer.isReady()) {
    ...
    strip.setPixelColor(4, Wheel(rainbowWheel & 255));
    strip.setPixelColor(5, Wheel(rainbowWheel & 255));
    strip.show();
    rainbowWheel++;
  }
}
